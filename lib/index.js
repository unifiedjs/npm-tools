...
async function organizationRun(context, npmOrg) {
  ...
  const groups = await pSeries(
    repositories.map((repo) => () => repoRun(context, repo, npmOrg))
  )
  ...
  for (const name of Object.keys(orgPackage)) {
    const info = packages.find((y) => name === y.name)
  ...
  for (const github of people) {
    const human = context.humans.find((h) => h.github === github)
...
async function packageRun(context, npmOrg, repo, manifestFilename) {
  ...
  return {
    ...
    private: Boolean(!packageData || !packageData.name || packageData.private),
    repo: repo.name
  }
}
