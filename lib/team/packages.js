...
async function packages(info) {
  ...
  const {name, permissions} = structure
  const {npmRequest, org, npmTokenOwner, packages: expected} = ctx
  ...
  await pSeries(
    expected
      .filter(x => {
        ...
        return !npmPkg || npmPkg.permissions !== permissions
      })
      .map(pkg => () => {
        return npmRequest
          .put(
            '/-/team/' +
              ...
              '/package',
            {
              body: {package: pkg.name, permissions}
            }
          )
          .then(() => {
            console.log(
