 ...
 * @import {Context, NpmRole} from '../util/types.js'
 */
...
export async function members(context) {
  const {humans, npmOrgs, npmOrg, npmTokenOwner, npmToken, org} = context
  ...
  console.error(chalk.bold('members') + ' for %s', org)

  const expected = people
    .map((github) => {
      const result = humans.find((h) => h.github === github)
      assert(result, 'Could not find human for @' + github)
      return result
    })
    .filter((human) => {
      const {npm} = human
  ...
  const body = /** @type {Record<string, NpmRole>} */ (await response.json())
